      ...
      try {
        if (access_token) {
          ...
          if (pdfs?.data) {
            ...
            // Handle case where pdfs.data is undefined or null
            console.error("Fetch returned undefined or null:", pdfs);
          }
        }
      } catch (error) {
        console.error("Error fetching pdfs:", error);
        setError("Failed to fetch PDFs. Please try again later."); // Set error state
      } finally {
  ...
  async function deletePdf(pdfId: string) {
    ...
    try {
      if (access_token) {
        ...
        console.log(pdfs);
        // Optionally update pdfData state after deletion if needed
                          ...
                          <Menu.Item>
                            {({ active }) => (
                              <button
                                ...
                                onClick={() => {
                                  deletePdf(project._id); // Pass the correct pdfId to deletePdf function
                                }}
                              >
                                Delete
                              </button>
